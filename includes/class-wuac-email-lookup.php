...
class WUAC_Email_Lookup {
     ...
     *
     * Validates input, parses the email list, and finds matching users.
     *
    ...
    public function process_email_lookup( string $raw_input ): array|string {
        if ( '' === trim( $raw_input ) ) {
            return __( 'Please enter at least one email address.', 'wp-user-audit-cleanup' );
        }

        $emails = $this->parse_email_list( $raw_input );

        if ( count( $emails ) > self::MAX_EMAILS ) {
            return __( 'The email list exceeds the maximum of 5000 entries. Please reduce the list and try again.', 'wp-user-audit-cleanup' );
        }
        ...
        $matched         = $this->find_matching_users( $emails );
        $matched_emails  = array_map( function ( $user ) {
            return strtolower( $user['user_email'] );
        }, $matched );
        $unmatched_count = count( array_diff( array_map( 'strtolower', $emails ), $matched_emails ) );
     ...
     * @return array Array of matched user data arrays (ID, user_login, user_email, user_registered).
     */
